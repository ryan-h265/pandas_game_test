...
class ShaderTester(ShowBase):
    ...
    def _create_test_geometry(self):
        ...
        if geom_type == "hemisphere":
            ...
            self.test_object.setPos(
                0, self.zoom_distance, 0
    ...
    def _load_shader(self):
        ...
        if shader:
            ...
            if self.shader_name == "sky":
                ...
                self.test_object.setShaderInput("time", 0.0)
                self.test_object.setShaderInput(
                ...
                # NodePath
                self.test_object.setShaderInput(
                    "shadowMatrix0", self.test_object.getMat(self.render)
                )
...
Examples:
  ...
  python shader_test.py terrain --plane     # Terrain shader on plane
    ...
    parser.add_argument(
        "shader", nargs="?", default="sky", help="Shader name to test (default: sky)"
    )

    # Geometry override options (mutually exclusive)
    geom_group = parser.add_mutually_exclusive_group()
    geom_group.add_argument(
        "--cube", action="store_const", const="cube", dest="geometry"
    )
    geom_group.add_argument(
        "--sphere", action="store_const", const="sphere", dest="geometry"
    )
    geom_group.add_argument(
        "--plane", action="store_const", const="plane", dest="geometry"
    )
    geom_group.add_argument(
        "--hemisphere", action="store_const", const="hemisphere", dest="geometry"
    )
