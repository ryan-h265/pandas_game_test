...
from testgame.menus import MenuManager
...
class Game(ShowBase):
    def __init__(self, saves_directory="saves"):
        ...
        # Initialize physics (needed early)
        self.setup_physics()

        # Setup lighting (needed early)
        self.setup_lighting()

        # Setup mouse control early
        self.mouse_captured = False

        # Start update task (will check if level is loaded)
        self.taskMgr.add(self.update, "update")

        self.saves_dir = Path(saves_directory)
        self.saves_dir.mkdir(exist_ok=True)

        # Initialize menu system and show start menu
    ...
    def list_saves(self):
        ...
        for save_file in self.saves_dir.glob("*.json"):
            print(f"Found save file: {save_file}")
            try:
    ...
    def return_to_main_menu(self):
        ...
        # Clean up current level/game state
        ...
        if hasattr(self, "character_model"):
            # self.character_model.cleanup()
            del self.character_model
        if hasattr(self, "terrain_editor"):
            del self.terrain_editor
        if hasattr(self, "hud"):
            # self.hud.cleanup()
            del self.hud
        if hasattr(self, "crosshair_manager"):
            del self.crosshair_manager
        if hasattr(self, "tool_manager"):
            # self.tool_manager.cleanup()
            del self.tool_manager
        if hasattr(self, "effects_manager"):
            del self.effects_manager
        if hasattr(self, "weapon_viewmodel"):
            del self.weapon_viewmodel
        if hasattr(self, "point_light_manager"):
            del self.point_light_manager
        if hasattr(self, "shadow_manager"):
            del self.shadow_manager
        if hasattr(self, "post_process"):
            del self.post_process

        self.destroy_input()

        # Hide pause menu and show start menu
        if self.menu_manager.pause_menu:
            self.menu_manager.pause_menu.hide()
        self.menu_manager.show_start_menu()
        print("Returned to main menu.")

    def load_level(self, level_key):
        """Load a specific level and initialize game systems.

        Args:
            level_key: Key from config.levels.LEVELS
        """
        ...
        # Initialize pause menu (after HUD and other systems are ready)
        self.menu_manager.init_pause_menu()
        ...
        # Setup mouse control and capture it
    ...
    def setup_input(self):
        ...
        # Pause menu
        self.accept("escape", self.menu_manager.toggle_pause)
    ...
    def toggle_chunk_colors(self):
        ...
        try:
            import testgame.config.settings as settings
    ...
    def quit_game(self):
        """Clean quit handler"""
        print("\nQuitting game...")
        if hasattr(self, "shadow_manager") and self.shadow_manager:
            self.shadow_manager.cleanup()
        self.userExit()
    ...
    def update(self, task):
        ...
        # Skip game logic if paused (start menu or pause menu active)
        if self.menu_manager.is_paused:
            # Update HUD if it exists (for FPS counter if enabled)
            if hasattr(self, "hud"):
                fps = globalClock.getAverageFrameRate()
                self.hud.update(dt, fps=fps)
            return task.cont

        # Skip if level not loaded yet
